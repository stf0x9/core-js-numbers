...
function getFibonacciNumber(index) {
  ...
  for (let i = 1; fibsArr.length <= index; i += 1) {
    const current = fibsArr[i];
    const prev = fibsArr[i - 1];

    fibsArr.push(current + prev);
  }
  return fibsArr[index];
}
...
function getSumOfNumbers(x1, x2, x3) {
  const arr = [x1, x2, x3];
  return arr.reduce((acc, curr) => acc + curr * 100, 0) / 100;
}
